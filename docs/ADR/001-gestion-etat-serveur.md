...
Accepté. Implémenté pour les consultations et écritures livrées dans les lots 1 et 2, puis étendu au cache OpenLibrary du lot 3.
...
Le lot 3 ajoute à la fiche une donnée OpenLibrary strictement secondaire. Son rythme de fraîcheur, ses erreurs et son identité par titre ne doivent pas se confondre avec les caches de l’API BookList.
...
Confier à TanStack Query l’état serveur des lots 1 à 3 : lectures, cache, états de requêtes et coordination des mutations.
...
- Conserver l’enrichissement OpenLibrary sous une famille de clés dédiée contenant le titre effectivement recherché, avec cinq minutes de fraîcheur et trente minutes de rétention. Une actualisation en échec conserve la dernière donnée exploitable sans produire d’erreur globale.
...
Le ticket #36 ajoute la famille `['openlibrary', 'recherche', titre]`. Le hook attend la stabilisation du titre, annule la lecture devenue inutile et n’expose une donnée que si elle correspond encore au titre demandé. Le service valide la réponse externe et utilise le transport HTTP commun avec un délai propre de cinq secondes. OpenLibrary ne met à jour aucune clé BookList.
...
- Une panne OpenLibrary peut laisser une donnée secondaire périmée visible, mais elle ne remplace jamais la fiche ni ses actions par un état d’erreur.
