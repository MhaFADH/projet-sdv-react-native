...
9. `FondsScreen` résout chaque couverture hors de la présentation avec `services/couvertures.ts`. Un chemin relatif rejoint l’API, une URL HTTP ou HTTPS absolue reste inchangée, une valeur absente produit une URL stable dérivée de l’identifiant et une valeur invalide sélectionne le visuel local.
