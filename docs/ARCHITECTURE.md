...
| `features/notes/`      | Coordonne la saisie d’une note de lecture : formulaire, verrouillage, issues d’écriture, abandon confirmé, et traduction de la requête des notes en états de présentation.                                                                                                                        |
...
4. `useAjouterNote` déclare `retry: false` : `POST /books/:id/notes` n’est pas idempotent et un second envoi créerait une seconde note.
