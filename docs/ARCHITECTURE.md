...
| Couche                 | Responsabilité livrée                                                                                                                                                                                                                                |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app/`                 | Compose Expo Router, TanStack Query, les providers globaux de préférences, de suppression et de bascules, le thème effectif et l’ErrorBoundary global. Détient la page consultée dans l’URL et déclenche les navigations. `app/+html.tsx` est l’enveloppe HTML de la version web, rendue à la seule génération du document : elle déclare `lang="fr"` pour les technologies d’assistance.                                                                                                              |
| `features/books/`      | Transforme l’état des hooks en états de présentation, pour le fonds paginé, sa sélection, la fiche et le formulaire partagé d’ajout et de correction. Interprète l’issue d’une écriture et coordonne les modifications collectives — bascules et notation — ainsi que le cycle global des suppressions différées.                                         |
| `features/preferences/` | Porte les préférences globales d’apparence et de langue : restauration au démarrage, choix explicites, thème effectif diffusé par Context et bascule de langue appliquée à l’internationalisation. |
...
6. `url-api.ts` lit et valide `EXPO_PUBLIC_API_URL`, puis construit les URLs du client HTTP et des médias relatifs. `clientHttp` ajoute les en-têtes communs, construit les paramètres et applique un délai d'expiration de dix secondes.
...
9. `FondsScreen` et `FicheScreen` résolvent chaque couverture hors de la présentation avec `services/couvertures.ts`. Un chemin relatif rejoint l’API, une URL HTTP ou HTTPS absolue reste inchangée, une valeur absente produit une URL stable dérivée de l’identifiant et une valeur invalide sélectionne le visuel local.
10. `FondsView` reçoit un état de chargement, d’erreur ou de succès, avec les couvertures déjà résolues. Il distingue fonds vide, critères sans résultat et chargement d’une autre page. Une barre d’outils unique empile les zones « Affiner » puis « Trier » sur écran large ; un panneau replié avec résumé la remplace sur petit écran. Les groupes radio conservent un seul arrêt de tabulation et répondent aux flèches, à Début et à Fin. Pendant le chargement d’une autre page, les anciennes lignes restent lisibles, mais leur sélection, la suppression et la pagination sont désactivées jusqu’à la réponse serveur. `CouvertureOuvrage` impose 80 × 120 points, demande à Expo Image un cache mémoire et disque, et remplace localement tout chargement en échec sans déclencher d’écriture.
11. Si une écriture concurrente fait disparaître la page demandée, `FondsScreen` revient à la dernière page indiquée par le serveur. L’état transitoire conserve une commande « Précédent » au lieu de présenter tout le fonds comme vide.
...
5. `FormulaireOuvrageScreen` tient l’URL de couverture de la saisie courante. `genererUrlCouverture` la produit une seule fois avec Faker, à l’ouverture du formulaire : ni un rendu, ni un échec, ni une bascule de thème ou de langue, ni une vérification du fonds ne la remplacent. Seule une création confirmée en prépare une nouvelle pour la saisie suivante. Faker fournit une URL fictive : il n’héberge aucune image, et une ressource inutilisable retombe sur le visuel local de `CouvertureOuvrage` lors des consultations ultérieures.
6. `createBook` envoie les cinq champs du lot 1 et cette `couverture` par le client HTTP commun, éditeur vide inclus comme chaîne et jamais comme `null`, puis valide la réponse `201` avec le même schéma Zod que les lectures.
7. En succès, l’ouvrage validé alimente la clé de sa fiche, les clés de liste sont invalidées de manière ciblée par leur préfixe, le formulaire est vidé, le statut revient à « Non lu » et un toast de cinq secondes propose la fiche créée. Aucune redirection automatique n’a lieu ; l’ouvrage reprend sa place dans le tri serveur au prochain chargement de la liste.
8. En échec, `interpreterEchecCreation` distingue trois issues. Un `422` alimente les champs concernés, même sans message général, sans toucher aux valeurs saisies. Une réponse `503` propose un réessai manuel après temporisation. Une requête sans réponse concluante — coupure ou délai d’expiration dépassé, reconnue par l’absence de statut HTTP — est présentée comme un résultat inconnu : l’ouvrage a peut-être été créé, aucun succès n’est annoncé, et le libraire choisit entre vérifier le fonds et réessayer en étant averti du risque de doublon. Aucune de ces trois issues n’efface la saisie ni ne change l’URL de couverture ; un réessai manuel renvoie donc la même valeur, ce qui ne rend pas `POST /books` idempotent pour autant.
...
## Parcours d’une écriture livré : les bascules et la notation

Les tickets #20 et #35 réunissent les deux bascules collectives d’un ouvrage — coup de cœur et statut de lecture — et sa notation derrière une coordination unique. Le chemin est route de composition → `FondsScreen` ou `FicheScreen` → `useBascules` → `BasculesProvider` → service de `PATCH` → client HTTP partagé → API. Le coup de cœur est une recommandation collective, sans attribution individuelle et indépendante du statut : un ouvrage non lu peut être un coup de cœur. La notation numérique reste distincte d’une note de lecture textuelle.
...
3. Une seule modification peut être en cours par ouvrage : tant qu’un envoi existe pour un identifiant, ses commandes cœur, lecture et notation sont verrouillées dans toutes ses vues et une nouvelle intention est ignorée. Les autres ouvrages restent pleinement interactifs.
4. Le service envoie `PATCH /books/:id` avec le seul champ visé, `{ favori }`, `{ lu }` ou `{ note }`, conformément au README de l’API. Le client partagé fournit l’URL, les en-têtes, l’expiration et la traduction des erreurs. `books-api.ts` valide la réponse complète avec Zod, vérifie son identifiant et conserve les données serveur utiles, dont `version`.
...
8. Un échec de la relecture qui suit un `PATCH` confirmé est distinct d’un refus : la valeur confirmée reste affichée, l’interface annonce l’échec d’actualisation et propose de réessayer la lecture. Elle n’annonce jamais l’annulation d’une écriture déjà acquise. Une relecture valide mais plus ancienne que la réponse du `PATCH` ne remplace pas cette dernière : les requêtes de fiche et de page réconcilient chaque ouvrage par sa version avant de publier les données. Chaque actualisation porte aussi la séquence de son écriture et ignore sa conclusion lorsqu’une modification plus récente a commencé. Dans le fonds, un échec de relecture est celui de la requête de liste et garde son propre message neutre.
...
10. Le `PATCH` d’une modification ne transmet pas d’`AbortSignal`. Annuler une écriture déjà partie en laisserait le résultat inconnu, ce que l’invariant du projet interdit d’ignorer silencieusement ; l’obsolescence est traitée par la séquence, pas par l’annulation. Les lectures, elles, restent annulables.

Les bascules et la notation déclarent `networkMode: 'always'`. Par défaut, TanStack Query met une mutation en pause lorsqu’il juge l’application hors ligne : la modification resterait sans résultat ni erreur et le verrou de l’ouvrage ne se libérerait jamais. Ce lot n’offrant aucune file d’attente hors ligne, un échec doit rester visible et réessayable plutôt que mis en attente silencieusement. Ce réglage est aujourd’hui limité à ces modifications ; l’étendre aux autres écritures livrées relève d’une décision du responsable.

Ce verrouillage traite la **concurrence locale** des bascules et de la notation d’un même poste : il empêche deux intentions simultanées sur un même ouvrage et l’écrasement d’une réponse par une autre. Il ne résout pas les écritures concurrentes de plusieurs libraires sur le serveur. Aucun en-tête `If-Match` n’est envoyé et aucune réponse `409` n’est interprétée dans ce parcours : le contrôle de version obligatoire et la résolution des conflits restent hors du lot 2.
...
## Préférences globales d’apparence et de langue

`PreferencesProvider` lit les deux préférences au montage par `services/plateforme/stockage-preferences`, dont Metro sélectionne l’implémentation `.web.ts`. Une valeur absente ou invalide retombe sur sa valeur initiale — thème « système » et français — sans bloquer l’application, la validation étant portée par `domain/preferences.ts`.

Le provider calcule l’apparence effective : le thème « système » suit `useColorScheme`, un choix explicite l’emporte. Il diffuse ensuite deux contextes distincts : `ThemeContext` porte le thème effectif consommé par la présentation via `useStylesTheme`, `PreferencesContext` porte l’état et les commandes consommés par l’écran Préférences. Les composants ne lisent jamais le stockage.

Un choix est appliqué immédiatement en mémoire puis persisté, en n’écrivant que la clé modifiée : choisir un thème ne réécrit jamais la langue enregistrée. Un choix explicite fait pendant la restauration est conservé, la restauration ne repeuplant que les préférences non encore choisies. La langue est transmise à i18next par `appliquerLangue`, ce qui met à jour à chaud toutes les chaînes rendues par `useTraduction`.

Le stockage n’avale aucune erreur : une lecture comme une écriture en échec — stockage bloqué, quota dépassé — remonte au provider, qui expose `stockageEchoue`. L’écran Préférences affiche alors une alerte et un réessai ; les choix restent appliqués pour la session.

Les contrôles de l’écran exposent rôle, libellé et état sélectionné, portent un indicateur de focus issu du token `focus` des deux palettes et respectent une cible de 44 points.

Les formats d’affichage suivent la langue active : `useFormats` dérive `fr-FR` ou `en-US` de la langue i18next et fournit le formatage des nombres ainsi que la locale passée à `formaterDateNote`. Les contenus métier — titres, auteurs, éditeurs, notes de lecture — ne passent jamais par l’internationalisation.
