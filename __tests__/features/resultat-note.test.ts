...
import { TEXTES_NOTE } from '../../features/notes/textes-note';
...
describe('interprétation d’un échec d’ajout de note', () => {
  ...
  it('traite une réponse illisible comme un résultat incertain', () => {
    expect(interpreterEchecAjoutNote({ type: 'validation', message: 'Réponse invalide.' })).toEqual(
      {
        type: 'incertain',
  ...
  it('traite une cause hors du modèle applicatif comme un résultat incertain', () => {
    expect(interpreterEchecAjoutNote(new Error('bruit'))).toEqual({
      type: 'incertain',
      message: TEXTES_NOTE.incertainReponseInexploitable,
    });
  });
  ...
  it('traite une absence de l’ouvrage comme un refus explicite', () => {
    expect(interpreterEchecAjoutNote({ type: 'introuvable', message: 'Livre inconnu.' })).toEqual({
      type: 'refus',
      parChamp: {},
