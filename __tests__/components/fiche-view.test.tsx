...
describe("présentation de la fiche", () => {
  it("affiche un squelette accessible pendant le chargement", () => {
    ...
    expect(
      screen.getByRole("progressbar", { name: "Chargement de la fiche" }),
    ).toBeVisible();
  ...
  it("contextualise l'absence de fiche sans inventer de contenu", () => {
    render(
      <FicheView
        etat={{ type: "introuvable", message: "Livre inconnu." }}
        retour={vi.fn()}
      />,
    );
  ...
  it("reste valide lorsque l'éditeur accepté par le contrat est vide", () => {
    render(
      <FicheView
        etat={etatSucces({ ...ouvrage, editeur: "" })}
        retour={vi.fn()}
      />,
    );
  ...
  it("annonce un statut de lecture négatif sans le déguiser", () => {
    render(
      <FicheView
        etat={etatSucces({ ...ouvrage, lu: false })}
        retour={vi.fn()}
      />,
    );
  ...
  it("affiche le chargement dans le bouton sans modifier son contenu dimensionnant", () => {
    ...
    expect(
      screen.getByLabelText("Enregistrement du statut en cours"),
    ).toBeVisible();
  ...
  it("rend la restauration et son réessai accessibles après un refus", () => {
    ...
    render(
      <FicheView
        etat={{
          ...
          erreurStatut: {
            message:
              "Le statut précédent a été restauré. Service indisponible.",
            reessayer,
          },
        }}
        retour={vi.fn()}
    ...
    expect(screen.getByRole("alert")).toHaveTextContent(
      "Le statut précédent a été restauré.",
    );
